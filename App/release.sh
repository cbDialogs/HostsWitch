#!/bin/zsh
# Builds a Developer ID signed, notarized, stapled HostsWitch DMG.
...
rm -f "$DMG"
hdiutil create -volname "HostsWitch" -srcfolder "$STAGE/dmg" -ov -format UDZO "$DMG"
...
UPLOAD_DIR=~/Library/Caches/hostswitch-release
mkdir -p "$UPLOAD_DIR"
cp "$DMG" "$UPLOAD_DIR/"
